...
- **教程章节拆分规则**：每个模块的每一章（如"01-音视频基础概念"）如果内容超过500行，必须拆分为多个小节文件放在子目录中（如 `01-音视频基础概念/01-什么是音视频.md`、`01-音视频基础概念/02-编码与容器.md`），而不是全塞进一个 .md 文件。所有已完成模块（P03-P06、M02-M03）均已完成拆分重构
...
| 模块 | 状态 | 文件数 | 备注 |
|------|------|--------|------|
| P01-现代CMake与构建工具链 | 🔍 待术语审查 | 16 md + README | 全部增强完成（350-500行/节），需检查术语零容忍 |
| P02-vcpkg包管理 | 🔍 待术语审查 | 10 md + README | 全部10文件完成（350-484行/节），需检查术语零容忍 |
| P03-跨平台C++开发 | 🔍 待术语审查 | 22 md + README | 5章拆分为子目录，每章3-5节，需检查术语零容忍 |
| P04-OpenGL图形编程 | 🔍 待术语审查 | 20 md + README | 8章拆分为子目录，用户已确认存在裸术语问题 |
| P05-软件渲染原理 | 🔍 待术语审查 | 15 md + README | 5章拆分为子目录，每章2-4节 |
| M01-项目导览与环境搭建 | 🔍 待术语审查 | 12 md + README | 全部增强完成（306-523行/节） |
| M02-项目构建系统深度解析 | 🔍 待术语审查 | 16 md + README | 5章拆分为子目录，每章3-4节 |
| M03-平台抽象层解析 | 🔍 待术语审查 | 18 md + README | 5章拆分为子目录，每章2-5节 |
| P06-FFmpeg音视频开发 | 🔍 待术语审查 | 26 md + README | 7章拆分为子目录，每章3-4节 |
| P07-OpenAL音频编程 | 🔍 待术语审查 | 14 md + README | 4章拆分为子目录，每章2-5节 |
| P08-编译原理基础 | 🔍 待术语审查 | 24 md + README | 7章拆分为子目录，每章3-4节（含MiniScript实战） |
| P09-逆向工程入门 | 🔍 待术语审查 | 19 md + README | 6章拆分为子目录，每章3-4节（含KiriKiri插件逆向实战） |
| P10-Cocos2d-x框架 | 🔍 待术语审查 | 16 md + README | 5章拆分为子目录，每章3-4节（含KrKr2 Cocos2d集成实战） |
| M04-渲染子系统 | 🔍 待术语审查 | 22 md + README | 7章拆分为子目录，每章3节 |
| M05-音频子系统 | 🔍 待术语审查 | 16 md + README | 5章拆分为子目录，每章3节（含FLAC解码器实战） |
| M06-视频播放器 | 🔍 待术语审查 | 18 md + README | 6章拆分为子目录，每章3节 |
