...
# Windows (PowerShell)
Get-ChildItem -Directory cpp/core/
```

对照本节列出的 9 个模块（tjs2、base、visual、environ、sound、movie、utils、plugin、extension），确认每个子目录都存在。

### 实践 2：CMake 目标关系验证

打开 `cpp/core/CMakeLists.txt`，找到 `add_library(krkr2core INTERFACE)` 和对应的 `target_link_libraries`，列出所有被聚合的子模块名称。与你在实践 1 中看到的目录一一对应。

### 实践 3：插件清单比对

打开 `cpp/plugins/CMakeLists.txt`，统计当前参与编译的插件数量。再打开 `doc/krkr2_plugins.md`，统计生态中已知插件总数。计算差值，理解"当前覆盖率"。
...
相关文件：
- `CMakeLists.txt` 第 46-84 行：顶层平台分支与可执行目标定义。
- `cpp/core/CMakeLists.txt` 全文：INTERFACE 目标 `krkr2core` 的聚合方式。
- `cpp/plugins/CMakeLists.txt` 全文：STATIC 目标 `krkr2plugin` 的聚合方式。
- `platforms/windows/main.cpp`、`platforms/linux/main.cpp`、`platforms/apple/macos/main.cpp`、`platforms/android/cpp/krkr2_android.cpp`：四平台入口。
- `tests/CMakeLists.txt`：测试构建的组织模式。
...
`cpp/core/base/` 不是"基础工具库"，而是包含归档格式（XP3/ZIP/TAR/7z）、流系统、事件系统、脚本管理等核心基础设施。如果只看目录名就假设它是"一些工具函数"，会严重低估其复杂度和重要性。

### 错误 2：把 INTERFACE 库当成"空目标"

看到 `add_library(krkr2core INTERFACE)` 就以为它没有实际代码。INTERFACE 是 CMake 的聚合手段，它把 9 个 STATIC 子模块链接成一个统一入口。代码在子模块里，INTERFACE 只负责传播链接关系。

### 错误 3：忽略 `cpp/external/` 的格式豁免

`cpp/external/` 下有 `.clang-format` 设置 `DisableFormat: true`，说明这些是第三方代码，不应修改格式。如果不知道这一点，可能会在格式化时意外修改第三方源码。

---

## 本节小结

- KrKr2 目录结构直接反映架构分层：core（9 模块）、plugins、platforms、tests。
- `krkr2core`（INTERFACE）和 `krkr2plugin`（STATIC）是两个关键聚合目标。
- 四平台入口（Windows/Linux/macOS/Android）均汇聚到 `TVPAppDelegate`。
- 测试采用"每 .cpp 一个可执行文件"的 Catch2 模式。
- 阅读目录时始终结合 `CMakeLists.txt` 理解真实依赖关系。

---

## 练习题与答案

### 题目 1：`krkr2core` 和 `krkr2plugin` 分别使用什么 CMake 库类型？为什么？

<details>
<summary>查看答案</summary>

`krkr2core` 使用 `INTERFACE` 类型——它本身不包含源码，只聚合 9 个 STATIC 子模块的链接关系并传播头文件路径。这样上层只需链接一个目标就能获得所有核心能力。

`krkr2plugin` 使用 `STATIC` 类型——插件代码直接编译进这个静态库，最终与主程序链接。这是因为 KrKr2 不使用动态加载插件的方式，所有插件在编译时静态绑定。

</details>

### 题目 2：Android 平台入口与桌面端入口有什么本质区别？

<details>
<summary>查看答案</summary>

桌面端（Windows/Linux/macOS）的入口是 `main()` 函数，产物是可执行文件。Android 的入口是 JNI 桥接函数，产物是共享库（`.so`），由 Java 层的 Activity 加载并调用。这意味着 Android 的启动流程多了 Java→JNI 的跨语言调用环节。

</details>

### 题目 3：如果要新增一个核心模块（比如 `network`），需要修改哪些 CMakeLists.txt？

<details>
<summary>查看答案</summary>

至少需要修改两处：
1. `cpp/core/network/CMakeLists.txt`（新建）——定义该模块的 STATIC 库、源文件和依赖。
2. `cpp/core/CMakeLists.txt`——在 `krkr2core` 的 `target_link_libraries` 中添加新模块，使其被聚合到核心目标中。

不需要修改根 `CMakeLists.txt`，因为根级只链接 `krkr2core`，新模块会通过 INTERFACE 传播自动包含。
...
进入下一节：[`02-CMake目标关系图.md`](./02-CMake目标关系图.md)。你将从"目录即架构"的视角切换到"CMake 目标即依赖图"的视角，用图表方式理解模块之间的真实依赖网络。
