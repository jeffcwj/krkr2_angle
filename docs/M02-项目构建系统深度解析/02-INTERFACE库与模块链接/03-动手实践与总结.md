...
### 错误 1：INTERFACE 库误用 target_sources 导致编译失败

**现象**：给 INTERFACE 库添加源文件后，`cmake --configure` 报错 `target_sources called with non-compilable target type`，或编译时所有链接此库的目标都重复编译相同源文件。

**原因**：INTERFACE 库是"纯接口"概念——它本身不编译任何代码。如果用 `target_sources(my_interface_lib INTERFACE file.cpp)` 添加源文件，每个链接此库的目标都会将 `file.cpp` 加入自己的编译列表，导致重复编译和潜在的 ODR（One Definition Rule）违反。

**解决**：

```cmake
# 错误：给 INTERFACE 库添加源文件
add_library(my_module INTERFACE)
target_sources(my_module INTERFACE impl.cpp)  # 所有链接者都会编译 impl.cpp！

# 正确方案 1：改用 STATIC 库（如果需要编译源文件）
add_library(my_module STATIC impl.cpp)
target_include_directories(my_module PUBLIC ${CMAKE_CURRENT_SOURCE_DIR})

# 正确方案 2：保持 INTERFACE 但只传递头文件
add_library(my_module INTERFACE)
target_include_directories(my_module INTERFACE ${CMAKE_CURRENT_SOURCE_DIR})
# 源文件在具体的 STATIC/SHARED 库中编译
```

> 项目中 `krkr2core` 是 INTERFACE 库（`cpp/core/CMakeLists.txt`），只传递 `target_link_libraries`，不包含源文件。实际源文件在各子模块（如 `tjs2`、`core_visual_module`）的 STATIC 库中编译。

### 错误 2：WHOLE_ARCHIVE 遗漏导致 Android JNI 注册失败

**现象**：Android 版本启动时崩溃，Logcat 报 `java.lang.UnsatisfiedLinkError: No implementation found for native method...`，但代码中明明用 `JNI_OnLoad` 或 `RegisterNatives` 注册了。

**原因**：链接器默认丢弃未被直接引用的符号。JNI 的 native 方法注册代码通常没有被 C++ 侧直接调用（而是通过 Java 层的动态查找），所以链接器认为它们是"无用代码"并优化掉了。

**解决**：

```cmake
# 错误：普通链接（JNI 注册代码被优化掉）
target_link_libraries(krkr2 PUBLIC krkr2plugin krkr2core)

# 正确：使用 WHOLE_ARCHIVE 强制保留所有符号
target_link_libraries(krkr2 PUBLIC
    "$<LINK_LIBRARY:WHOLE_ARCHIVE,krkr2plugin;krkr2core>"
)
# WHOLE_ARCHIVE 告诉链接器：不要丢弃这些库中任何符号

# 注意：LINK_LIBRARY 生成器表达式需要 CMake 3.24+
cmake_minimum_required(VERSION 3.24)
```

> 项目 `CMakeLists.txt` 第 115-117 行正是使用了 `WHOLE_ARCHIVE` 来确保所有 JNI 注册代码和插件初始化代码不被丢弃。

### 错误 3：循环依赖导致 CMake 配置死循环

**现象**：`cmake --configure` 时卡死或报错 `Circular dependency detected for target...`。

**原因**：模块 A 的 CMakeLists.txt 中 `target_link_libraries(A PUBLIC B)`，同时模块 B 中 `target_link_libraries(B PUBLIC A)`，形成循环依赖。这在大型项目中不易察觉，尤其是通过 INTERFACE 库间接传播时。
...
# 正确方案：提取共同依赖到独立模块 C
# C/CMakeLists.txt: 
add_library(C STATIC common.cpp)
# A/CMakeLists.txt:
...
> 项目通过 INTERFACE 聚合库模式避免了循环依赖：各子模块（tjs2、visual 等）互不依赖，全部通过 `krkr2core` 聚合后再链接到最终目标。
