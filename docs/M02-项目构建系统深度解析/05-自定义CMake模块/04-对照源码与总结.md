...
### 错误 1：cmake_parse_arguments 参数名拼写错误静默忽略

**现象**：自定义 CMake 函数调用时传入了某个参数，但函数内部读取该参数的值为空。没有任何错误提示。

**原因**：`cmake_parse_arguments` 只匹配预定义的参数名列表。如果调用者传入了不在列表中的参数名（通常是拼写错误），`cmake_parse_arguments` 会将其放入 `<prefix>_UNPARSED_ARGUMENTS`，但不会报错。函数内部使用正确的参数名读取时自然为空。
...
function(my_func)
    cmake_parse_arguments(
        ARG                    # 前缀
        "VERBOSE"              # 布尔选项
        "TARGET;OUTPUT_DIR"    # 单值参数
        "SOURCES;DEPENDS"      # 多值参数
        ${ARGN}
    )
    
    # 关键：检查未解析参数（防止拼写错误被静默忽略）
    if(ARG_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR 
            "my_func: 未识别的参数: ${ARG_UNPARSED_ARGUMENTS}")
    endif()
    
    # 检查必需参数
    if(NOT ARG_TARGET)
        message(FATAL_ERROR "my_func: TARGET 参数是必需的")
    endif()
endfunction()

# 调用时拼写错误会立即报错：
my_func(TRAGET my_app SOURCES main.cpp)
# FATAL_ERROR: my_func: 未识别的参数: TRAGET;my_app
```

> 项目 `CocosBuildHelpers.cmake` 中的函数普遍使用了 `cmake_parse_arguments`，但未检查 `UNPARSED_ARGUMENTS`——这是可以改进的地方。

### 错误 2：function 与 macro 作用域混淆导致变量泄漏或丢失

**现象**：在 `function()` 中设置的变量，调用者无法读取；或者在 `macro()` 中设置的临时变量污染了调用者的作用域。

**原因**：`function()` 创建独立作用域，内部设置的变量不会影响外部（除非使用 `PARENT_SCOPE`）。`macro()` 在调用者作用域展开，内部设置的所有变量都直接修改调用者的变量。

**解决**：

```cmake
# 错误：function 中设置变量但忘记 PARENT_SCOPE
function(get_platform_name result_var)
    if(WINDOWS)
        set(${result_var} "win64")   # 只在 function 作用域内生效
    endif()
endfunction()
get_platform_name(PLAT_NAME)
message("平台: ${PLAT_NAME}")   # 输出空！

# 正确：使用 PARENT_SCOPE 将变量传递给调用者
function(get_platform_name result_var)
    if(WINDOWS)
        set(${result_var} "win64" PARENT_SCOPE)  # 传递到调用者作用域
    endif()
endfunction()

# 或者改用 macro（如果确实需要修改调用者变量）
macro(get_platform_name result_var)
    if(WINDOWS)
        set(${result_var} "win64")   # 直接在调用者作用域中设置
    endif()
endmacro()
```

### 错误 3：cocos_copy_target_dll 在非 MSVC 编译器下找不到 DLL

**现象**：使用 MinGW 或 Clang 在 Windows 上构建时，运行程序报找不到 DLL，但使用 MSVC 构建时正常。

**原因**：`cocos_copy_target_dll` 使用 `get_target_property(IMPORTED_LOCATION)` 查找 DLL 路径。MSVC 构建的 vcpkg 包会设置 `IMPORTED_LOCATION` 指向 `.dll` 文件，但 MinGW 构建的包可能将 DLL 放在不同的目录结构中（如 `bin/` vs `lib/`）。
...
# 手动指定 DLL 搜索路径
if(MINGW)
    # MinGW vcpkg 包的 DLL 通常在 installed/x64-mingw-dynamic/bin/
    set(DLL_SEARCH_PATH "${VCPKG_INSTALLED_DIR}/${VCPKG_TARGET_TRIPLET}/bin")
    
    # 构建后将所有 DLL 拷贝到输出目录
    add_custom_command(TARGET krkr2 POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_directory
            "${DLL_SEARCH_PATH}"
            "$<TARGET_FILE_DIR:krkr2>"
    )
endif()
```

> 项目目前只支持 MSVC 构建 Windows 版本，所以 `cocos_copy_target_dll` 的逻辑针对 MSVC 优化。如果未来支持 MinGW，需要扩展此函数。
