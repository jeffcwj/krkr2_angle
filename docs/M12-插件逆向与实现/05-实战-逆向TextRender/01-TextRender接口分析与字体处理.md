...
### 2.4 setOption 内部处理函数分析

`sub_10005980` 不仅注册了 `setOption` 方法名，还在注册时绑定了两个内部处理函数，分别在 `setOption` 被调用时处理不同的属性键：

#### sub_100019F0 — "following" 属性处理器（2093 字节）

```asm
; sub_100019F0 — setOption 对 "following" 键的处理逻辑
; 调用者：sub_10005980（0x100059AF）
; 关键引用地址

0x10001A56: push offset aFollowing      ; "following"
; 这行反汇编直接暴露了属性键的字符串常量
; sub_100019F0 负责从 tTJSVariant 中提取 "following" 键对应的值
; 并将其保存到 TextRenderBase 对象的对应成员变量
```

`"following"` 在 KiriKiri TextRender 语境下表示**跟随模式**（Following Mode）——一种让文字渲染位置跟随上一个字符末端的排版特性。当 `following` 为 `true` 时，新绘制的字符从上次渲染结束的位置继续（类似"追加模式"），常用于实现逐字打印效果（Typewriter Effect，文字一个接一个地出现）。

#### sub_10002220 — "face" 属性处理器（1674 字节）

```asm
; sub_10002220 — setOption 对 "face" 键的处理逻辑
; 调用者：sub_10005980（0x100059D8）
; 关键引用地址

0x10002286: push offset aFace           ; "face"
0x1000228B: push offset aFace           ; "face"（第二次引用，做字符串比对）
; sub_10002220 从 tTJSVariant 中提取 "face" 键，读取字体面名称字符串
; 并调用内部字体加载逻辑更新当前活跃字体
```

`"face"` 是字体名称属性（Font Face Name）——即字体的英文或中文全名，如 `"MS Gothic"`、`"SimSun"`。

此外，`sub_100028B0`（另一个字体相关处理函数，1674 字节）在 `0x10002916` 和 `0x1000291C` 处也引用了 `"face"` 字符串，说明 `face` 属性至少在两处被处理：一处读取（`sub_10002220`），一处可能是验证或应用（`sub_100028B0`）。

#### 完整的 setOption 调用关系图

```
sub_10005980 (ncbind 注册函数, 3310 字节)
  │
  ├── 0x100059AF: call sub_100019F0 (2093B)  ← 注册 "following" 属性处理器
  ├── 0x100059D8: call sub_10002220 (1674B)  ← 注册 "face" 属性处理器
  ├── 0x100059C3: push "setOption"           ← 注册 setOption 方法名
  ├── 0x100059EC: push "setDefault"          ← 注册 setDefault 方法名
  ├── 0x10005A0D: push "setRenderSize"       ← 注册 setRenderSize 方法名
  └── 0x10005C30: call sub_10010260 (2773B)  ← 注册行级渲染函数
```

这意味着在实现 TextRender 的 `setOption` 方法时，需要处理至少两个属性键：

```cpp
// TextRender::setOption 实现骨架（根据 IDA 逆向推断）
void TextRenderBase::setOption(const tTJSVariant& opt) {
    // opt 是一个 TJS 字典对象（tTJSVariant of type tvtObject）
    // 通过枚举字典键来处理各属性
    
    // 处理 "following" 键 → sub_100019F0 的逻辑
    tTJSVariant followingVal;
    if (opt.GetProperty(L"following", &followingVal) == TJS_S_OK) {
        following_ = (bool)followingVal;  // 是否启用跟随模式
    }
    
    // 处理 "face" 键 → sub_10002220 的逻辑
    tTJSVariant faceVal;
    if (opt.GetProperty(L"face", &faceVal) == TJS_S_OK) {
        face_ = (const wchar_t*)faceVal;  // 字体名称
        // 触发字体重新加载
        reloadFont();
    }
    
    // 还可能有其他属性键（从 RTTI 的 PropertyCommand 推断）
    // size, bold, color, italic, spacing 等
}
```
...
// 属性名来源：sub_10005980 反汇编（方法名字符串）+ sub_100019F0/sub_10002220 内部处理器
class TextRenderBase : public tTJSNativeInstance {
public:
    // —— 属性（Property） ——

    // face — 字体面名称（Font Face Name）
    // 真实字符串来源：0x10002286 "face"（sub_10002220 内部）
    // 例如 "MS Gothic"、"SimSun"、"Meiryo"
    const wchar_t* get_face() const;
    void set_face(const wchar_t* fontName);

    // following — 跟随模式（Following Mode）
    // 真实字符串来源：0x10001A56 "following"（sub_100019F0 内部）
    // true = 下次 drawText 从上次渲染结束位置继续（追加模式）
    // false = 使用绝对坐标（默认）
    bool get_following() const;
    void set_following(bool enabled);
    ...
    // —— 方法（Method） ——
    // setOption — 批量设置选项（通过 TJS 字典传入多个键值对）
    // 内部处理器：sub_100019F0（"following"）、sub_10002220（"face"）
    void setOption(tTJSVariant option);
...
该函数内部调用 `sub_100121A0`（4,402 字节）**3 次**（调用地址分别在 `0x1000FB4E`、`0x1000FF33`、`0x1000FFE5`），`sub_100121A0` 是逐字符渲染的内部循环——这个调用方向从 IDA 的交叉引用可以直接确认：`sub_100121A0` 的所有调用者均来自 `sub_1000DFE0`。
...
[esi+1C4h]     std::vector::_Myfirst        0x1000E070, 0x10010299 — data/begin 指针
[esi+1D0h]     std::vector::_Mylast         0x1000E05B — end 指针（已填充部分末端）
[esi+1D4h]     std::vector::_Myend          0x1000E063, 0x100102C1 — capacity 末端指针
```

> **MSVC std::vector 三指针布局说明**：MSVC 的 `std::vector<T>` 内部存储 3 个指针而非"data + size + capacity"三个值：
> - `_Myfirst`（`1C4h`）：分配内存的起始地址（`data()` 返回这个值）
> - `_Mylast`（`1D0h`）：最后一个已构造元素之后的位置（`end()` 返回这个值；`size() = (_Mylast - _Myfirst) / sizeof(T)`）
> - `_Myend`（`1D4h`）：分配内存的末端（`capacity() = (_Myend - _Myfirst) / sizeof(T)`）
>
> 这与常见误解"vector 存储 data/size/capacity"不同——MSVC 存储的是三个原始指针。逆向时看到 `cmp [reg+1D0h], [reg+1D4h]`（即 `_Mylast == _Myend`）就是在检查 vector 是否已满（size == capacity）。

`[esi+1C4h]` / `[esi+1D0h]` / `[esi+1D4h]` 这组偏移量的访问模式几乎可以确定是 `std::vector` 的内部结构。这很可能是**字形缓存**（Glyph Cache）——一个存储已渲染字形数据的动态数组。
...
IDA 分析数据文件：
- `stuffs/ida_scripts/textrender_analysis.txt` — 2490 行完整分析报告（导出表、导入表、函数列表、RTTI、反汇编）
