# Android NDK 与 JNI 动手实践与总结

> **所属模块：** P03-跨平台C++开发
> **前置知识：** [NDK 基础与 JNI 类型系统](./01-NDK基础与JNI类型系统.md)、[JNI 函数注册与双向调用](./02-JNI函数注册与双向调用.md)、[引用管理与字符串处理](./03-引用管理与字符串处理.md)、[KrKr2 Android 启动与按键映射](./04-KrKr2-Android启动与按键映射.md)
> **预计阅读时间：** 20 分钟

## 本节目标

读完本节后，你将能够：
1. 独立编写一个完整的 JNI 函数（Java 声明 + C++ 实现 + CMakeLists.txt）
2. 从 C++ 调用 Android 系统 API（如获取 SDK 版本号）
3. 识别并修复 JNI 开发中最常见的 3 类错误（命名、线程、引用）
4. 对照 KrKr2 源码理解实际项目中的 JNI 架构分层
