...
> **所属模块：** P03-跨平台C++开发
> **前置知识：** [日志与 CMake 构建](./04-日志与CMake构建.md)、[文件系统接口与实现](./02-文件系统接口与实现.md)、[线程接口与实现](./03-线程接口与实现.md)
> **预计阅读时间：** 20 分钟

## 本节目标

读完本节后，你将能够：
1. 编写跨平台统一测试代码，使用简易测试宏验证 PAL 各模块的正确性
2. 在 Windows/Linux/macOS 上构建并运行 Mini-PAL 项目
3. 识别并解决跨平台构建中的 3 个常见错误（链接库缺失、宏冲突、符号未定义）
4. 为 Mini-PAL 添加新功能（文件复制、动态库编译）
5. 对照 KrKr2 源码理解每个 PAL 模块的设计决策
