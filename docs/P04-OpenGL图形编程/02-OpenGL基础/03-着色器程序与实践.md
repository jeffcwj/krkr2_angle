...
> **前置知识：** [缓冲对象与VBO](./01-缓冲对象与VBO.md)、[VAO与EBO](./02-VAO与EBO.md)  
> **预计阅读时间：** 40 分钟
...
1. 编写最小的顶点着色器与片段着色器，并理解两者在渲染管线中的角色。
2. 完成着色器的**编译→附加→链接**三步流程，并正确获取编译/链接错误日志。
3. 使用 `glUseProgram` + `glBindVertexArray` + `glDrawArrays`/`glDrawElements` 完成一次完整绘制。
4. 排查常见的着色器与绘制问题：黑屏、数据错乱、EBO 丢失。
5. 对照 KrKr2 项目源码理解真实引擎中着色器程序的创建与使用方式。

## 着色器程序基础

在现代 OpenGL 中，必须提供**着色器程序**（Shader Program）才能绘制任何东西。着色器是运行在 GPU 上的小程序，至少需要**顶点着色器**（Vertex Shader，负责处理每个顶点的位置和属性）和**片段着色器**（Fragment Shader，负责计算每个像素的最终颜色）两个阶段。
...
void main() {
    gl_Position = vec4(aPos, 1.0);   // 输出裁剪空间坐标（clip space，经投影变换后的坐标，各分量在 [-w, w] 范围内，GPU 据此裁剪不可见部分）
...
下一节 [GLSL 语言与着色器阶段](../03-着色器编程/01-GLSL语言与着色器阶段.md) 将深入讲解 GLSL 着色器语言，包括数据类型、变量限定符、内置函数，以及如何编写高质量的跨平台着色器代码。
